...
void				load_numbers_from_file(const char *filename,
						struct s_Node **head);

struct s_Node		*find_lowest_unprocessed_number(struct s_Node *head);
int					process_lowest_unprocessed_number(struct s_Node *head,
						int newValue);
...
void				counting_sort(struct s_Node **head, int bit);
void				radix_sort(struct s_Node **head);
...
void				move_stacks(struct s_Node **Stack_A,
						struct s_Node **Stack_B, int bitIndex);
